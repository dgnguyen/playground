export const calPointsBaseball = (ops): number => {
  const result = ops.reduce((acc, cur, index) => {
    if (cur === '+' && index > 1) {
      return [...acc, Number(acc[acc.length - 1]) + Number(acc[acc.length - 2])]
    }
    if (cur === 'C' && index > 0) {
...
export function findCommonKeys(obj1: any, obj2: any) {
  ...
  for (const key of commonKeys) {
    if (typeof obj1[key] === 'object' && typeof obj2[key] === 'object') {
      ...
      commonKeys.push(
        ...nestedCommonKeys.map(nestedKey => `${key}.${nestedKey}`),
      )
    }
  }
...
export function climbingLeaderboard(
  ...
  const rankedToRank = sortIndexArr.reduce((acc, cur, i) => {
    if (
      sortIndexArr[i + 1]?.value &&
      sortIndexArr[i + 1]?.value !== cur.value
    ) {
      return [
        ...acc,
        {
  ...
  const playerRank: number[] = player.reduce((acc, cur) => {
    ...
    const minRankIndex = sortRankedToRank.findLastIndex(
      item => item.value > cur,
    )
    const minRank = sortRankedToRank[minRankIndex].rank
    return [...acc, minRank + 1]
  }, [])
  return playerRank
}
